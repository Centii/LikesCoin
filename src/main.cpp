...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Likescoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Likescoin Signed Message:\n";
...
{
    // Likescoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Likescoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Likescoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Likescoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Likescoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Likescoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Likescoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LikescoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LikescoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LikescoinMiner(CWallet *pwallet)
{
    printf("LikescoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("likescoin-miner");
    ...
    try { loop {
        ...
        printf("Running LikescoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LikescoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LikescoinMiner, pwallet));
}
